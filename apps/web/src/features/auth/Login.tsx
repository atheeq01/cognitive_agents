...
export const Login: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-background px-4">
      ...
      {!IS_FIREBASE_CONFIGURED && import.meta.env.DEV && (
        <div className="w-full max-w-md mb-6 p-4 bg-yellow-500/10 border border-yellow-500/50 rounded-xl text-yellow-600 dark:text-yellow-500 text-sm">
          <p className="font-bold flex items-center gap-2">
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
            Firebase Not Configured
          </p>
          <p className="mt-1">Real Google Sign-In won't work. Copy <code>.env.example</code> to <code>.env</code> and add your keys.</p>
        </div>
      )}

      <div className="w-full max-w-md space-y-8 p-8 border rounded-xl shadow-lg bg-card text-card-foreground">
        <div className="text-center">
          <h2 className="text-3xl font-bold tracking-tight">OmniMind v2</h2>
        ...
        <div className="mt-6">
          ...
          <div className="mt-6 space-y-3">
            <button
              ...
              disabled={loading}
              className="w-full flex items-center justify-center py-2 px-4 border border-border rounded-md shadow-sm bg-background text-sm font-medium text-foreground hover:bg-muted focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary disabled:opacity-50"
            >
